...
class Agenda extends CI_Controller
{
	...
	public function list()
	{
		...
		$config['base_url'] = base_url('agenda/list');
		$config['total_rows'] = $this->agenda_model->count_agenda();
		$config['per_page'] = 12;
		...
		$data = $this->agenda_model->all($config['per_page'], $url);
		// echo "<pre>";
		// print_r($data);die;
		// echo "</pre>";
		$data['url'] = $url;
	...
	{
		$data = $this->agenda_model->save($id);
		$this->load->view('index', ['data'=>$data]);
	}

	public function ubah_status($id=0)
	{
		$data = $this->agenda_model->ubah_status($id);
		$this->load->view('index', ['data'=>$data]);
	}
	...
	public function delete($id=0)
	{
		if(!empty($id))
		{
			$data = $this->agenda_model->delete($id);
			$this->load->view('index', ['data'=>$data]);
		}
	}
}